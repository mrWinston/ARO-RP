...
func New(ctx context.Context, log *logrus.Entry, _env env.Interface, db database.OpenShiftClusters, dbGateway database.Gateway, dbOpenShiftVersions database.OpenShiftVersions, aead encryption.AEAD,
	...
	armPrivateEndpoints, err := armnetwork.NewPrivateEndpointsClient(_env.Environment(), r.SubscriptionID, fpCredential)
	if err != nil {
		return nil, err
	}

	armSecurityGroupsClient, err := armnetwork.NewSecurityGroupsClient(_env.Environment(), r.SubscriptionID, fpCredential)
	if err != nil {
		return nil, err
	}

	armRPPrivateLinkServices, err := armnetwork.NewPrivateLinkServicesClient(_env.Environment(), r.SubscriptionID, msiCredential)
	if err != nil {
		return nil, err
	}

	armSubnetsClient, err := armnetwork.NewSubnetsClient(_env.Environment(), r.SubscriptionID, fpCredential)
	if err != nil {
		return nil, err
	}
